...
/** Recurrence interval used only when explicitly marking a task complete —
 *  never applied automatically. Year-End and Confirmation Statement are
 *  annual, VAT Return quarterly. */
const TASK_RECURRENCE_MONTHS: Record<DueDateTask, number> = {
  year_end: 12,
  confirmation_statement: 12,
  vat_return: 3,
}

/** The next occurrence once a task is marked complete — the current due
 *  date rolled forward by its recurrence interval. This only ever runs
 *  from an explicit "Mark completed" action; the due date never changes
 *  on its own just because time has passed. */
export function computeNextCycle(task: DueDateTask, currentDueDate: Date): Date {
  return addMonthsClamped(currentDueDate, TASK_RECURRENCE_MONTHS[task])
}
